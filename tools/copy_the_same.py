...
def merge_the_base(base, files):
  for f in files:
    ...
    for entry in jsonfile:
      ...
      eng = entry["Texts"]["Eng"]
      if eng in base and ("Por" not in entry["Texts"] or
                          len(entry["Texts"]["Por"]) == 0):
        newentry["Texts"]["Por"] = base[eng]
      newjson += [newentry]
    f.seek(0)
    f.truncate()
